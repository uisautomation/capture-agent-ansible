...
class FilterModule:
    def filters(self):
        return {
            'friendly_hostname': generate_friendly_hostname,
        }


def generate_friendly_hostname(s):
    """
    ...
    # the range (0.0, 1.0). We then map that float into an index into the total
    # list of possible names. To guard against the unlikely scenario of ending
    # up with the float 1.0, we restrict the range of the generated index.
    total_name_count = len(ADJECTIVES) * len(NOUNS)
    digest_float = float(
        fractions.Fraction(int(digest, 16), int('f' * len(digest), 16))
    )
    name_index = min(
        total_name_count-1,
        int(math.floor(total_name_count*digest_float))
    )

    # Convert the index over all possible names into an actual name
    adjective = ADJECTIVES[name_index // len(NOUNS)]
    noun = NOUNS[name_index % len(NOUNS)]

    # For added entropy, use the reversed digest to generate a three-digit
    # number for the host. We use the reversed digest so that the number is
    # non-linearly related to the name_index.
    reverse_digest_float = float(
        fractions.Fraction(int(digest[::-1], 16), int('f' * len(digest), 16))
    )
    index = 100 + min(899, int(math.floor(900 * reverse_digest_float)))

    return '{}-{}-{}'.format(adjective, noun, index)


# These adjectives and nouns are taken from
...
# The Docker source code includes the following plea: "please, for any amazing
# man that you add to the list, consider adding an equally amazing woman to it,
# and vice versa."
NOUNS = [
